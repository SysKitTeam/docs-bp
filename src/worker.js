/**
 * Cloudflare Worker for serving Docusaurus static site on /bp route
 * This worker handles routing and serves static assets for testforce.uk/bp
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    // Handle requests to /bp route
    let pathname = url.pathname;
    
    // If the path starts with /bp, remove it for internal routing
    if (pathname.startsWith('/bp')) {
      pathname = pathname.replace('/bp', '') || '/';
    } else {
      // Return 404 for non-/bp routes to maintain consistency
      return new Response('Not Found - Please access via /bp route', { status: 404 });
    }
